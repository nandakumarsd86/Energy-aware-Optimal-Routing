function [best_fitness, best_solution, Convergence_curve, time] = DFA(X, sphere_function, lb, ub, max_iterations)
...
while iter < max_iterations - 10
    ...
    for i = 1:size(advanced, 1)
        if r3 < r2
            ...
            ref_highest = highest(randi(size(highest, 1)), :);
            D = norm(ref_highest - advanced(i, :));
            advanced(i, :) = advanced(i, :) + D * exp(b * rand * (2 * pi)) .* cos(2 * pi * rand(1, dim));
        end
